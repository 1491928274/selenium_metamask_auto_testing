...
def addAndChangeNetwork():
    ...
    driver.get('chrome-extension://{}/home.html'.format(EXTENSION_ID))
    # driver.refresh()
    driver.find_element(By.XPATH, "//button[text()='批准']").click()
    driver.find_element(By.XPATH, "//button[text()='切换网络']").click()
    time.sleep(3)
    driver.close()
    driver.switch_to.window(driver.window_handles[0])
...
def changeNetworkByChainList(network_name):
    ...
    driver.find_element(By.XPATH, "//h5[text()='Connect Wallet']").click()
    # connect chainlist
    driver.execute_script("window.open('');")
    ...
    driver.switch_to.window(driver.window_handles[1])
    # search Network
    driver.find_element(By.XPATH, "//span[text()='Testnets']").click()
    time.sleep(1)
    inputs = driver.find_element(By.XPATH, '//input')
    inputs[0].send_keys(network_name)
    time.sleep(1)
    driver.find_element(By.XPATH, "//span[text()='Add to Metamask']").click()
    # change Network
    time.sleep(3)
    driver.execute_script("window.open();")
    driver.switch_to.window(driver.window_handles[2])
